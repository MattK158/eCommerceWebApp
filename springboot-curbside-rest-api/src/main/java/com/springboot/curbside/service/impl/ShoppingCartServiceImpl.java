...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public ShoppingCartDTO createShoppingCart(ShoppingCartDTO shoppingCartDto) {
        ...
        CustomerDTO customerDto = customerService.getCustomerById(shoppingCartDto.getCustomerId());
        if(customerDto == null)
        {
    ...
    private double calculateDollarValueOfCart(List<ItemInCartDTO> items){
        ...
        for(ItemInCartDTO item: items){
            if(item.getDiscount().equalsIgnoreCase("NONE"))
            {
                totalAmount+=Double.parseDouble(item.getPrice());
            }
            else {
                totalAmount += (Double.parseDouble(item.getPrice()) * (1 - (Double.parseDouble(item.getDiscount()))));
            }
        }

        return 1.0825 * totalAmount;
    }
    ...
    @Transactional
    public void removeAllItemsFromCustomerCart(long customerId)
    {
